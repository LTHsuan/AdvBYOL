...
class CW(object):
    def __init__(self, model,norm, IsTargeted, kappa, lr, init_const, max_iter, binary_search_steps, data_name):
    ...
    def forward(self, xs=None, ys=None, ytarget=None, device=None):
        ...
        if targeted:
            ...
            # targeteg_class_in_one_hot = torch.stack(targeteg_class_in_one_hot).clone().type_as(xs) #torch.Size([10, 10])
            ...
            # targets_in_one_hot = torch.stack(targets_in_one_hot).clone().type_as(xs) #torch.Size([10, 10])
        ...
        for search_for_c in range(self.binary_search_steps):
            ...
            modifier = Variable(modifier.to(self.device), requires_grad=True)
            optimizer = torch.optim.Adam([modifier], lr=self.learning_rate)
            # var_const = const_origin.clone()
            var_const = const_origin.clone().to(self.device)
            print("\tbinary search step {}:".format(search_for_c))

            for iteration_times in range(self.max_iter):
                # inverse the transform tanh -> [0, 1]
                # print(var_xs.type(), modifier.type(), half_range.type(), mid_point.type())
                ...
                if targeted:
                    constraint_loss = torch.max((prediction - 1e10 * targeteg_class_in_one_hot).max(1)[0] - (prediction * targeteg_class_in_one_hot).sum(1),
                        ...
                        # torch.ones(batch_size) * self.kappa * -1,
                    )
                else:
        ...
        # adv_xs = best_perturbation
        adv_xs = best_perturbation.to(device)

        outputs = self.net(adv_xs)
        adv_labels = torch.argmax(outputs, axis=-1)
        print(adv_labels, ys)
        #print("FGSM:", torch.argmax(outputs, axis=-1),labels)
        success = (adv_labels != ys).cpu().numpy()
        print("cw:{}/{}".format(np.sum(success), len(success)))
