...
class PGD(object):
    ...
    def __init__(self, net, epsilon, norm, stepsize, steps, data_name,target,loss):
        self.epsilon = epsilon
        self.p = norm
        self.net = net
        self.stepsize = stepsize        #alpha
        self.steps = steps              #iteration
        self.loss = loss
        self.target = target
    ...
    def forward(self, image, label, target_labels):
        ...
        for i in range(self.steps):
            ...
            loss = loss_adv(self.loss, netOut, label, target_labels, self.target)        
        ...
        adv_label = torch.argmax(outputs, axis=-1)
        success = (adv_label != label).cpu().numpy()
        print("PGD:{}/{}".format(np.sum(success), len(success)))
        return advimage