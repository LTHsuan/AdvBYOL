...
def loss_adv(loss_name, outputs, labels, target_labels, target):
    if loss_name=="ce":
        ...
        if target:
            one_hot_labels = torch.eye(len(outputs[0]))[target_labels]
...
def margin_loss(outputs, labels, target_labels, targeted):
    if targeted:
        one_hot_labels = torch.eye(len(outputs[0]))[target_labels]
        #one_hot_labels = torch.eye(len(outputs[0]))[target_labels].to(device)
        i, _ = torch.max((1-one_hot_labels)*outputs, dim=1)
